...
def generate_vid():
    alphabet = "abcdefghijklmnpq1234567890"  # Custom alphabet
    return f"vid_{shortuuid.ShortUUID(alphabet=alphabet).random(length=7)}"

def generate_payout_id():
    alphabet = "abcdefghijklmnpq1234567890"  # Custom alphabet
    return f"poid_{shortuuid.ShortUUID(alphabet=alphabet).random(length=7)}"
...
class Vendor(models.Model):
    ...
    vid = models.CharField(max_length=20, unique=True, default=generate_vid)
...
class Payout(models.Model):
    ...
    item = models.ForeignKey("core.OrderItem", on_delete=models.SET_NULL, null=True, related_name="item")
    amount = models.DecimalField(max_digits=12, decimal_places=2, default=0.00)
    payout_id = models.CharField(max_length=20, unique=True, default=generate_payout_id)
...
class Notifications(models.Model):
    ...
    order = models.ForeignKey("core.OrderItem", on_delete=models.CASCADE, null=True, blank=True)
