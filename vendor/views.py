...
def products(request):
    ...
    products = paginate_queryset(request, products_list,20) #Pagination
...
def orders(request):
    ...
    orders_list = Order.objects.filter(vendors=request.user, payment_status = 'Paid')
...
@admin_required
def reviews(request):
    reviews_list = Review.objects.filter(product__vendor=request.user)
...
def notifications(request):
    notis_list = Notifications.objects.filter(user=request.user, seen=False).order_by('-date')
...
def update_product(request, id):
    ...
    if request.method == "POST":
        ...
        if variant_ids and variant_titles:
            ...
            for i, variant_id in enumerate(variant_ids):
                variant_name = variant_titles[i].title().strip()
                ...
                if variant_id:  
                    ...
                    variant =  Variant.objects.create(product=product, name=variant_name)
                
                # items for this variant
                ...
                for j in range(len(item_titles)):
                    item_title = item_titles[j].title().strip()

                    # skip only if title is empty
                    if not item_title:
                        continue

                    item_id = item_ids[j] if j < len(item_ids) else ""
                    item_description = item_descriptions[j].strip() if j < len(item_descriptions) else ""

                    if item_id and item_id.isdigit():
                        variant_item = VariantItem.objects.filter(id=item_id).first()
                        if variant_item:
        ...
        for file_key, image_file in request.FILES.items():
            if file_key.startswith('image_'):  
                 Gallery.objects.create(product=product, image=image_file)
    ...
    context = {
        ...
        'gallery_images':  Gallery.objects.filter(product=product),  # Pass existing gallery images to the template
    }
...
def delete_variants(request, product_id, variant_id):
    ...
    variants =  Variant.objects.get(product__vendor=request.user, product=product, id=variant_id)
    variants.delete()
...
def delete_variants_items(request, variant_id, item_id):
    ...
    item =  VariantItem.objects.get(variant=variant, id=item_id)
    item.delete()
...
def delete_product_image(request, product_id, image_id):
    product =  Product.objects.get(id=product_id)
    image =  Gallery.objects.get(product=product, id=image_id)
    image.delete()
